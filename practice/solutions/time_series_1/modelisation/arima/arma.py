...
def statsmodels_modelisation(
    data: pd.DataFrame, target_name: str, p: int = 1, q: int = 1
) -> pd.DataFrame:
    ses_model = ARIMA(data.loc[:, target_name], order=(p, 0, q)).fit()
...
if __name__ == "__main__":
    ...
    statsmodels_preds = statsmodels_modelisation(
        data=modelled_data, target_name="co2", p=4, q=2
    )
